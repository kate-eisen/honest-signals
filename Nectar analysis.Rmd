...
nectar_work <- nectar %>% filter(!is.na(Population)) %>%  filter(!is.na(nectar_μl)) %>% filter(!is.na(Corola_area_mm2)) %>% filter(Corola_area_mm2>0)
...
nectar_work8$Population <- factor(nectar_work8$Population, levels=c("Aal04",  "Aal12",  "Aal29",  "Aal34c", "Aal36" , "AalDMB", "AalFC",  "AalSFH", "E3"  ,   "Fr1"  ,  "Fr2" ,   "Fr3"  , 
 "Fr4"  ,"S1",  "G3"   ,  "G4"  ,   "G5"   ,  "It10"  , "It13"  , "It15"  , "It16" ,  "It17"  , "It18" ,  "It2" ,   "It4", "It5",    "It6"  ,  "It8" ,   "It9"      ))

ggplot(aes(x=Corola_area_mm2, y=nectar_μl, color=Mating_system), data=nectar_work8)+geom_point()+
  facet_wrap(~factor(Population), scales="free")+theme_classic(base_size = 16)+xlab(expression(paste("Corolla area (", mm^2 ,")"))) + ylab(expression(paste("Nectar volume (", mu,"l)")))+geom_smooth(data=lines, method="lm", color="black") +scale_color_discrete(name="Mating system", breaks=c("SI","SC"), labels=c("Self-incompatible", "Self-compatible")) +theme(legend.position = "bottom") 
